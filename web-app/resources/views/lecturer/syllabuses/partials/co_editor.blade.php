...
<div class="space-y-6">
    <div class="flex items-end justify-between">
        ...
        <form method="POST" action="{{ route('lecturer.syllabuses.ai.generateCO', $syllabus->id) }}">
            @csrf
            <button
                class="bg-primary-container text-on-primary-container px-4 py-2 rounded-lg flex items-center gap-2 font-semibold text-xs"
                type="submit">
    ...
    @if (session('ai_result'))
        @php
            ...
            $score = floatval($ai['verification']['final_score'] ?? 0.85);
            $scorePercent = min(100, max(0, round($score * 100)));
        @endphp
                ...
                <div class="flex items-center gap-3">
                    <div class="relative w-12 h-12">
                        <svg class="w-full h-full transform -rotate-90">
                            <circle class="text-surface-container-high" cx="24" cy="24" fill="transparent"
                                r="20" stroke="currentColor" stroke-width="3">
                            </circle>
                            <circle class="text-primary-container" cx="24" cy="24" fill="transparent"
                                r="20" stroke="currentColor" stroke-dasharray="125.6"
                                stroke-dashoffset="{{ 125.6 - 125.6 * min(1, max(0, $score)) }}" stroke-width="3">
                            </circle>
                        </svg>
                        <span
                            class="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-primary">
                            {{ $scorePercent }}
                        </span>
                    </div>

                    <div>
                        <p class="text-[10px] text-primary font-bold">{{ $scorePercent }}% Match</p>
                        <p class="text-[9px] text-on-surface-variant/60">
                            {{ strtoupper($ai['verification']['status'] ?? 'PENDING') }}
                        </p>
                    </div>
                </div>
            ...
            <div class="mt-3 flex justify-end">
                <form method="POST" action="{{ route('lecturer.syllabuses.ai.acceptCO', $syllabus->id) }}">
                    ...
                    @foreach ($ai['generated_data'] as $index => $co)
                        <input type="hidden" name="course_objectives[{{ $index }}][code]"
                            value="{{ $co['code'] }}">
                        <input type="hidden" name="course_objectives[{{ $index }}][description]"
                            value="{{ $co['description'] }}">
                    @endforeach
                    <button type="submit"
                        class="px-3 py-1 bg-primary text-on-primary rounded text-[10px] font-bold">Accept</button>
                </form>
            </div>
        </div>
    @endif
    ...
    <div>
        <div class="flex items-center gap-2 mb-3">
            ...
            <span
                class="px-2 py-0.5 rounded bg-secondary-container/30 text-secondary text-[9px] font-label">{{ $syllabus->courseObjectives->count() }}</span>
        </div>
        ...
        @if ($syllabus->courseObjectives->count())
            <form method="POST" action="{{ route('lecturer.syllabuses.updateCO', $syllabus->id) }}" class="space-y-3">
                ...
                <div class="flex justify-end">
                    <button type="submit"
                        class="bg-primary-container text-on-primary-container px-4 py-2 rounded text-xs font-bold">Save
                        COs</button>
                </div>
            </form>
        @else
