...
<main class="ml-64 pt-24 px-8 pb-12 relative z-10 technical-grid min-h-screen">
    <div class="max-w-[1600px] mx-auto">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-surface-container-high/50 border-b border-outline-variant/10">
                            ...
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-outline">Môn học</th>
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-outline">CTĐT</th>
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-outline">Khung</th>
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-outline">Năm học</th>
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-outline">Trạng thái</th>
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-outline">Giảng viên tham gia</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-outline-variant/5">
                        ...
                        <tr class="group hover:bg-surface-container-low transition-colors">
                            ...
                            <td class="px-6 py-5">
                                <div class="space-y-1">
                                    <div class="font-label text-xs font-bold text-primary">{{ $syllabus->course->course_code ?? 'N/A' }}</div>
                                    <div class="text-sm text-on-surface">{{ $syllabus->course->course_name ?? 'N/A' }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <div class="space-y-1">
                                    <div class="font-label text-[10px] text-outline uppercase">{{ $syllabus->course->program->code ?? 'N/A' }}</div>
                                    <div class="text-xs text-on-surface-variant max-w-[200px]">{{ $syllabus->course->program->name ?? 'N/A' }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <span class="inline-flex items-center px-2 py-1 rounded bg-secondary-container/20 text-secondary text-[10px] font-bold uppercase tracking-wider">
                                    {{ $syllabus->template->template_name ?? 'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-5">
                                <span class="font-mono text-sm text-on-surface">{{ $syllabus->academic_year }}</span>
                            </td>
                            <td class="px-6 py-5">
                                @php
                                ...
                                <span class="inline-flex items-center px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border {{ $statusClass }}">
                                    <span class="w-1 h-1 rounded-full mr-1.5 {{
                                        $statusName == 'Draft' ? 'bg-warning' :
                                        ($statusName == 'Pending' ? 'bg-secondary' :
                                        ($statusName == 'Approved' ? 'bg-primary' :
                                        ($statusName == 'Rejected' ? 'bg-error' : 'bg-outline')))
                                    }}"></span>
                                    {{ $statusName }}
                                </span>
                            </td>
                                ...
                                <div class="space-y-1.5">
                                    @forelse($syllabus->assignments as $assignment)
                                        <div class="flex items-center gap-2 text-xs">
                                            ...
                                            <span class="text-on-surface font-medium">{{ $assignment->user->full_name ?? 'N/A' }}</span>
                                            <span class="text-[10px] text-outline uppercase px-1.5 py-0.5 rounded bg-surface-container-highest">
        ...
        <!-- Footer / Navigation & Technical Specs -->
        <div class="mt-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-t border-outline-variant/10 pt-6">
            <div class="flex items-center gap-6">
                <a href="{{ route('program_director.dashboard') }}"
            ...
            <div class="flex gap-8">
                <div class="flex flex-col">
                    <span class="font-label text-[9px] uppercase tracking-widest text-outline">Total Records</span>
                    <span class="text-[10px] font-bold text-on-surface">{{ $syllabuses->count() }} Syllabus{{ $syllabuses->count() != 1 ? 'es' : '' }}</span>
                </div>
                <div class="flex flex-col">
                    <span class="font-label text-[9px] uppercase tracking-widest text-outline">Last Sync</span>
                    <span class="text-[10px] font-bold text-primary">{{ now()->format('H:i:s') }}</span>
                </div>
            </div>
            <div class="flex items-center gap-3">
