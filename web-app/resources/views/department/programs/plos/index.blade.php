...
@section('title', 'Danh sách PLO | Syllabus_System')
...
@section('content')
    ...
    <div class="max-w-7xl mx-auto">
        ...
        @if (session('success'))
            <div
                class="mb-8 p-4 bg-primary-container/10 border-l-2 border-primary-container rounded-r-xl flex items-center gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    <thead>
                        <tr class="bg-surface-container-high/50 border-b border-outline-variant/10">
                            ...
                            <th class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-on-surface-variant">
                                Mô tả chi tiết</th>
                            <th
                                class="px-6 py-4 font-label text-[10px] uppercase tracking-widest text-on-surface-variant text-right">
                                Thao tác hệ thống</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($plos as $plo)
                            <tr class="hover:bg-surface-container-low transition-colors group">
                                <td class="px-6 py-5 align-top">
                                    <span
                                        class="font-label text-sm font-bold text-primary px-2 py-1 bg-primary/10 rounded border border-primary/20">
                                        {{ $plo->code }}
                                    </span>
                                </td>
                                <td class="px-6 py-5">
                                ...
                                <td class="px-6 py-5 text-right">
                                    <div class="flex items-center justify-end gap-3">
                                        ...
                                        <a class="p-2 rounded-lg bg-surface-container hover:bg-surface-container-highest text-on-surface-variant hover:text-on-surface transition-all flex items-center justify-center group/btn"
                                            ...
                                            <span
                                                class="max-w-0 overflow-hidden group-hover/btn:max-w-xs group-hover/btn:ml-2 transition-all duration-300 font-label text-[10px] uppercase tracking-tighter">Quản
                                                lý PI</span>
                                        </a>
                                        ...
                                        <a class="p-2 rounded-lg bg-surface-container hover:bg-surface-container-highest text-on-surface-variant hover:text-on-surface transition-all flex items-center justify-center group/btn"
                                            ...
                                            <span
                                                class="max-w-0 overflow-hidden group-hover/btn:max-w-xs group-hover/btn:ml-2 transition-all duration-300 font-label text-[10px] uppercase tracking-tighter">Sửa</span>
                                        </a>

                                        <!-- Xóa -->
                                        <form
                                            action="{{ route('department.programs.plos.destroy', [$program->id, $plo->id]) }}"
                                            method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button
                                                class="p-2 rounded-lg bg-error-container/10 hover:bg-error-container text-error transition-all flex items-center justify-center group/btn"
                                                onclick="return confirm('Xác nhận xóa PLO này?')" type="submit">
                                                <span class="material-symbols-outlined text-sm">delete</span>
                                                <span
                                                    class="max-w-0 overflow-hidden group-hover/btn:max-w-xs group-hover/btn:ml-2 transition-all duration-300 font-label text-[10px] uppercase tracking-tighter">Xóa</span>
                                            </button>
                                        </form>
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td class="px-6 py-24 text-center" colspan="3">
                                    <div class="flex flex-col items-center gap-4 opacity-40">
                                        ...
                                        <p class="font-label text-sm uppercase tracking-widest">Chưa có PLO nào trong hệ
                                            thống</p>
        ...
        <div class="mt-12 flex items-center justify-between border-t border-outline-variant/10 pt-6">
            <div class="flex gap-8">
                <div class="flex flex-col">
                    <span class="font-label text-[9px] uppercase tracking-widest text-on-surface-variant">System
                        Status</span>
                ...
                <div class="flex flex-col">
                    <span class="font-label text-[9px] uppercase tracking-widest text-on-surface-variant">Active
                        Objects</span>
