...
@section('title', 'Tạo Mới PLO | Syllabus_System')
...
@section('content')
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-on-surface-variant max-w-2xl">Khởi tạo một Program Learning Outcome (PLO) mới để ánh xạ với các
                năng lực cốt lõi của chương trình đào tạo.</p>
        </div>
        ...
        <div class="grid grid-cols-12 gap-8">
            ...
            <div class="col-span-12 lg:col-span-8">
                <section class="bg-surface-container rounded-xl p-8 border border-outline-variant/10 shadow-sm">
                    ...
                    @if ($errors->any())
                        <div class="mb-8 terminal-accent bg-error-container/10 p-4 rounded-r-lg border-l-2 border-error">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <h4 class="text-sm font-semibold text-error uppercase tracking-wider font-label">Cảnh
                                        báo: Lỗi xác thực</h4>
                    ...
                    <form action="{{ route('department.programs.plos.store', $program->id) }}" class="space-y-8"
                        method="POST">
                        @csrf

                        <!-- PLO Code Field -->
                        <div class="space-y-2">
                            <label class="block font-label text-[10px] uppercase tracking-widest text-outline font-bold"
                                for="code">
                                Mã định danh PLO
                            </label>
                            <input
                                class="w-full bg-surface-container-lowest border-0 rounded-lg p-4 text-on-surface shadow-inset-soft focus:ring-1 focus:ring-primary-container transition-all outline-none font-['Space_Grotesk']"
                                id="code" name="code" placeholder="Ví dụ: PLO-101" type="text"
                                value="{{ old('code') }}" />
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea
                                class="w-full bg-surface-container-lowest border-0 rounded-lg p-4 text-on-surface shadow-inset-soft focus:ring-1 focus:ring-primary-container transition-all outline-none resize-none"
                                id="description" name="description"
                                placeholder="Mô tả năng lực mà sinh viên sẽ đạt được sau khi hoàn thành chương trình..." rows="6">{{ old('description') }}</textarea>
                            <div class="flex justify-between items-center px-1">
                                <span class="text-[10px] text-outline italic">Khuyến nghị phân tích mật độ ngữ nghĩa.</span>
                                <span class="text-[10px] text-outline">{{ strlen(old('description', '')) }} / 500 ký
                                    tự</span>
                            </div>
                        </div>
                        ...
                        <div class="pt-6 flex items-center gap-4">
                            <button
                                class="bg-primary-container text-on-primary-container px-8 py-3 rounded-lg font-bold shadow-extruded flex items-center gap-2 hover:scale-[1.02] transition-transform"
                                type="submit">
            ...
            <div class="col-span-12 lg:col-span-4 space-y-6">
                <!-- AI Insight Card -->
                <div
                    class="bg-surface-container-high rounded-xl p-6 border border-primary-container/20 relative overflow-hidden group">
                    ...
                    <p class="text-sm text-on-surface-variant leading-relaxed">
                        Tôi đã phân tích cấu trúc chương trình hiện tại của bạn. Với chương trình <span
                            class="text-secondary font-mono">{{ $program->name }}</span>,
                        hầu hết các PLO đều tập trung vào <span class="italic">kiến thức nền tảng</span>.
                        Hãy cân nhắc thêm PLO liên quan đến <span class="text-on-surface underline decoration-primary/50">kỹ
                            năng thực hành và ứng dụng thực tế</span>.
                    </p>
                ...
                <div class="bg-surface-container rounded-xl overflow-hidden border border-outline-variant/10">
                    <div class="h-40 bg-surface-container-high relative">
                        ...
                        <div class="absolute bottom-4 left-4">
                            <span
                                class="text-[10px] font-label uppercase tracking-widest text-primary font-bold">{{ $program->name }}
                                - Blueprint</span>
                        </div>
                    </div>
                    <div class="p-4">
    ...
    <div class="fixed w-64 h-64 bg-primary-container/10 rounded-full blur-[80px] pointer-events-none z-50 transition-opacity duration-300 opacity-0"
        id="cursor-glow"></div>
        ...
        inputs.forEach(input => {
            ...
            if (input.id === 'description') {
                input.addEventListener('input', (e) => {
                    const counter = input.closest('.space-y-2')?.querySelector(
                        '.flex.justify-between.items-center .text-outline:last-child');
                    if (counter) {
